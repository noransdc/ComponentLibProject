...
public class DatePickerView extends BaseDatePickerView {
    ...
    protected void onFinishInflate() {
        ...
        rootLl = findViewById(R.id.ll_date_root);
    }

    @Override
    protected int getDatePickerViewLayoutId() {
        return R.layout.lib_dialog_date_picker;
    }
    ...
    public void setLanguageEn(boolean isEn){
        mYearWv.setEn(isEn);
        mMonthWv.setEn(isEn);
        mDayWv.setEn(isEn);
        ...
        try {
            if (isEn){
                rootLl.removeView(mYearWv);
                rootLl.removeView(mYearTv);
                rootLl.removeView(mDayWv);
                rootLl.removeView(mDayTv);

                rootLl.addView(mYearWv);
                rootLl.addView(mYearTv);
                rootLl.addView(mDayTv, 0);
                rootLl.addView(mDayWv, 0);
                ...
                rootLl.addView(mYearTv, 0);
                rootLl.addView(mYearWv, 0);
                rootLl.addView(mDayWv);
                rootLl.addView(mDayTv);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * 设置月份范围
     */
    public void setMonthRange(int start, int end){
        mMonthWv.setMonthRange(start, end);
    }
